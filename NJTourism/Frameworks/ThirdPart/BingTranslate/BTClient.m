...
- (void) addSuggestions:(NSArray *)suggestions forArray:(NSArray *)array fromLanguage:(NSString *)from toLanguage:(NSString *)to userName:(NSString *)user {
	if ([array count] != [suggestions count]) {
		...
		[_delegate bingTranslateClient:self errorOccurred:error];
		return;
	}
	NSString *userName = (user != NULL) ? user : [[NSBundle mainBundle] bundleIdentifier];
...
- (void) connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	if ([result hasPrefix:@"\"ArgumentOutOfRangeException:"]) {
		...
		if ([type isEqualToString:BTServiceTranslateText]) {
			// single translation
			[result unescapeJson];
			[_delegate bingTranslateClient:self translatedText:[dict objectForKey:@"original"] translation:result];
		} else if ([type isEqualToString:BTServiceTranslateArray]) {
			// multiple translation
			NSArray *resultArray = [result translationResults];
			[_delegate bingTranslateClient:self translatedArray:[dict objectForKey:@"original"] translations:resultArray];
			...
			[_delegate bingTranslateClient:self addedSuggestion:[dict objectForKey:@"suggestion"] forText:[dict objectForKey:@"original"] fromLanguage:[dict objectForKey:@"from"] toLanguage:[dict objectForKey:@"to"] userName:[dict objectForKey:@"user"]];
		} else if ([type isEqualToString:BTServiceSuggestArray]) {
			// suggest multiple texts
			// we don't expect any response (there isn't even any)
			// so just notify the delegate
			[_delegate bingTranslateClient:self addedSuggestions:[dict objectForKey:@"suggestion"] forArray:[dict objectForKey:@"original"] fromLanguage:[dict objectForKey:@"from"] toLanguage:[dict objectForKey:@"to"] userName:[dict objectForKey:@"user"]];
		} else if ([type isEqualToString:BTServiceDetectLanguageText]) {
			// detect language of a text
			NSString *text = [dict objectForKey:@"original"];
			[result unescapeJson];
			[_delegate bingTranslateClient:self detectedLanguage:result ofText:text];
			...
			[_delegate bingTranslateClient:self detectedLanguages:languages ofArray:texts];
			...
			[_delegate bingTranslateClient:self willStartSpeakingText:[dict objectForKey:@"original"] inLanguage:[dict objectForKey:@"language"]];
        ...
        {
			NSArray *langs = [result JSONValue];
			[_delegate bingTranslateClient:self receivedTranslationLanguages:langs];
		} else if ([type isEqualToString:BTServiceGetSpeechLanguages]) {
			NSArray *langs = [result JSONValue];
			[_delegate bingTranslateClient:self receivedSpeechLanguages:langs];
		}
	}
	// clean up
	[context removeObjectForKey:key];
...
- (void) audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag
{
	if (flag == YES)
    {
		[_delegate bingTranslateClientFinishedSpeaking:self];
	}
    else
    {
