...
- (NSData*)dataForURL:(NSURL*)aURL shouldLoadWithObserver:(id<EGOImageLoaderObserver>)observer
{
    ...
    NSData *imageData = [[EGOCache currentCache] dataForKey:keyForURL(aURL, nil)];
...
- (void)imageLoadConnectionDidFinishLoading:(EGOImageLoadConnection *)connection 
{
    ...
    NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)connection.response;
    
    UIImage *anImage = [UIImage imageWithData:responseData];
    
    BOOL isDataIntegrated = NO;
	
	if(!anImage)
    {
        NSString *domain = [connection.imageURL host]?[connection.imageURL host]:@"url error";
		NSError* error = [NSError errorWithDomain:domain code:406 userInfo:nil];
    ...
    else 
    {
        ...
        if ([httpResponse respondsToSelector:@selector(allHeaderFields)]) 
        {
            ...
            NSString *lengthNumber = [httpHeader objectForKey:@"Content-Length"];
            
            contentLength = [lengthNumber integerValue];
            
        }
        
        if (contentLength == realDataLength) 
        {
            
            isDataIntegrated = YES;
		...
		NSNotification* notification = [NSNotification notificationWithName:kImageNotificationLoaded(connection.imageURL)
																	 object:self
																   userInfo:[NSDictionary dictionaryWithObjectsAndKeys:responseData,@"data",
                                                                       connection.imageURL,@"imageURL",
                                                                       [NSNumber numberWithBool:isDataIntegrated],@"dataIntegrated",
                                                                             nil]];
