...
- (void)setPhoto:(id <EGOPhoto>)aPhoto{
	...
	if (self.photo != nil) {
		[[EGOImageLoader sharedImageLoader] removeObserver:self forURL:self.photo.URL];
	}
	...
	if (self.photo.image) {
		...
		if ([self.photo.URL isFileURL]) {
            ...
            NSData* imageData = [[EGOImageLoader sharedImageLoader] 
                                 dataForURL:self.photo.URL 
