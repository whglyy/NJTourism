...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if (_callback)
        {
            [_callback performSelector:@selector(handleError:WithConnection:) withObject:statusError withObject:self];
        }
    }
}
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    if (_callback)
    {
        [_callback performSelector:@selector(handleData:WithConnection:) withObject:_bufData withObject:self];
    }
}
@end
