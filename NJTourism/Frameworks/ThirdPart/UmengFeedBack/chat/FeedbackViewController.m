...
- (void)customizeNavigationBar:(UINavigationBar *)bar
{
    ...
    {
        [navBar setBackgroundImage:[UIImage imageNamed:@"sys_navbar_bg.png"] forBarMetrics:UIBarMetricsDefault];
    }
    else
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:
                                  [UIImage imageNamed:@"sys_navbar_bg.png"]];
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = L(@"用户反馈");
    ...
    [feedbackClient setAppkey:UmengFeedBackUserId delegate:(id<UMFeedbackDataDelegate>)self];
...
- (void)keyboardWillHide:(NSNotification *) notification
{
    ...
    if (bottomBarFrame.origin.y < 300)
    {
        ...
        [UIView setAnimationDuration: animationDuration];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
        bottomBarFrame.origin.y = self.view.bounds.size.height - 44;
        self.mToolBar.frame = bottomBarFrame;
        [UIView commitAnimations];
    }
}
